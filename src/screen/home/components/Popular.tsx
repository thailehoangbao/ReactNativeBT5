...
import { ParamsApi, fetchDataApi } from '../../api/RapidApi'
...
export default function Popular() {
  ...
  const [dataApi,setDataApi] = useState([
      {
        ...
        job_highlights: {
          ...
          Responsibilities: ["Starting 6/5, you will be working in an agile team to help develop a new set of web applications for their own system and sub vendors","This role is hybrid and you will be required to go in to the office 3 days p/w"],
          Benefits: ["6-month contract (likely extension)","W2 - $70 - $75 p/h","C2C - $80 - $90 p/h"]
        },
        job_job_title:null,
  ...
  useEffect(() => {
    fetchDataApi(params).then(result => {
      setDataApi(result)
    })
  ...
  const renderItem = (item: any) => {
    return <TouchableOpacity style={[style.mt16, style.ml24,{backgroundColor: "#e8e4da",padding:8,borderRadius: 8,width: 200}]} onPress={() => {OnPressDetail(item)}}>
      <View style={[style.btn,style.bgLightGray]} >
        <Image source={{uri:item.employer_logo}} style={style.icons} />
      </View>
      <Text style={style.mt8}>
        {item.employer_name}
      </Text>
  ...
  return (
    <View style={[style.mt16, { padding: 16 }]}>
      <View style={style.popularContainer}>
        <Text style={style.titlePopular}>Popular Job</Text>
